...
const COLUMNS = [
    ...
    { id: 'interviewing', title: 'Interviewing', color: 'bg-indigo-50 text-indigo-600', icon: User },
...
const ApplicantCard = ({ applicant, onStatusChange }) => {
    ...
    return (
        <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 mb-3 relative hover:shadow-[var(--shadow-md)] transition-all duration-300 group">
            <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[var(--color-surface-secondary)] flex items-center justify-center text-[var(--color-text-secondary)] font-bold border border-[var(--color-border)] overflow-hidden relative text-xs">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-wider mt-0.5">Applied {new Date(applicant.appliedAt).toLocaleDateString()}</p>
                    </div>
                </div>
                    ...
                    {isMenuOpen && (
                        <>
                            ...
                            <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-[var(--shadow-md)] z-20 py-1.5 backdrop-blur-md bg-white/90">
                                <p className="px-3 py-2 text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-widest border-b border-[var(--color-border)] mb-1">Update Status</p>
                                {COLUMNS.map(col => (
            ...
            <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2 text-xs font-medium text-[var(--color-text-secondary)]">
                    <Mail size={13} className="text-[var(--color-text-tertiary)]" />
            ...
            <div className="pt-3 border-t border-[var(--color-border)]">
                <div className="flex items-center justify-between">
                    <span className="px-2.5 py-1 rounded-lg bg-[var(--color-surface-secondary)] text-[10px] font-bold text-[var(--color-text-secondary)] border border-[var(--color-border)]">
                ...
                {aiMatch && (
                    <div className="mt-3 p-2.5 bg-gray-50 rounded-lg text-[10px] text-[var(--color-text-secondary)] leading-relaxed font-medium border border-gray-100 italic">
                        "{aiMatch.analysis}"
                    </div>
                )}
            </div>
...
const KanbanColumn = ({ column, applicants, onStatusChange }) => {
    return (
        <div className="flex flex-col w-[85vw] sm:w-80 min-w-[17rem] sm:min-w-[20rem] bg-[var(--color-bg)] rounded-2xl border border-[var(--color-border)] h-full max-h-[85vh] snap-start shadow-[var(--shadow-sm)]">
        ...
        <div className="flex flex-col h-full space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--color-surface)] p-6 rounded-2xl border border-[var(--color-border)] shadow-[var(--shadow-sm)]">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="p-2.5 hover:bg-[var(--color-surface-secondary)] rounded-xl transition-all border border-[var(--color-border)] text-[var(--color-text-secondary)] shadow-sm hover:shadow-md">
                        <Filter size={18} />
                    </button>
                </div>
            </div>

            <div className="flex gap-6 overflow-x-auto pb-8 scroll-smooth snap-x snap-mandatory scrollbar-thin">
                {COLUMNS.map(col => (
                    <KanbanColumn
                        key={col.id}
                        column={col}
